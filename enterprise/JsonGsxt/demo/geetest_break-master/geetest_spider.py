...
from helper.common import execJS
...
def get_gt_challenge(url):
    ...
    try:
        res = requests.get(url, headers=headers, cookies=cookies).content
...
def crawl_by_key(searchword):
    ...
    try:
        res = requests.get(test_url, headers=headers, cookies=cookies).content
        print res
        if res == 'false':
