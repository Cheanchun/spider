...
def data_parse(resp, sec_cate='', third_cate=''):
    source_data = resp.get('results')[0].get('hits')
    for item in source_data:
        ...
        if temp_id.isdigit():
            ...
            _id = max(res, key=len)
            index_product['color'] = max(res, key=len)
            index_product['product_id'] = temp_id.replace(_id, '')
        ...
        for key, value in p_url.items():
            ...
            index_product['product_url'] = urljoin('https://www.centrepointstores.com/', url)
            sec_cate, third_cate = key.split('-')[-2], key.split('-')[-1]
            break
        ...
        if item.get('sibiling'):
            for sibiling_item in item.get('sibiling'):
                temp_id = sibiling_item.get('code')
                if temp_id.isdigit():
                    index_product['product_id'] = temp_id
                else:
                    res = re.findall(r'[a-zA-Z]+', temp_id)
                ...
                save_data(index_product)
    return resp.get('results')[0].get('nbPages')
...
def main():
    file_d = './old_data'
    for params in os.listdir(file_d):
        for item in os.listdir(os.path.join(file_d, params)):
            file_path = os.path.join(file_d, params, item)
            try:
                with open(file_path, mode='r', encoding='u8') as fp:
                    data_json = json.loads(fp.read())
                    data_parse(data_json)
