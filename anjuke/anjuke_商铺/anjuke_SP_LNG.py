...
if not r.exists(REDIS_KEY):
    ...
    print(urls, len(urls))
with open("./{}AJK_CD_SP.json".format(datetime.datetime.today().date()), mode="a+", encoding="utf-8") as fp:
    while r.exists(REDIS_KEY):
        url = r.rpop(REDIS_KEY).decode("utf-8")
        print("-----------------  开始爬取url:%s  ------------------------" % url)
        ...
        for item in items:
            ...
            if "1" in floor or "一" in floor:
                ...
                if lat and lng:
                    ...
                    data["lat"] = ""
                try:
                    data["address"] = item.xpath("./dl/dd[2]/span//text()")[0][
                                      1:-1]  # item.xpath("./dl/dd[2]/span//text()")
                except:
                    continue
                #  出租信息 :
                ...
                # json.dump(data, fp, encoding="utf-8", ensure_ascii=False, skipkeys=True)
                fp.write(data + ",\n")
            else:
