...
class AnJuKe:
    ...
    def __init__(self):
        self.headers = None
        self.r = redis.StrictRedis(host=HOST, port=PORT, password=PWD)
    ...
    def _paras_list_page_item(self, resp):
        ...
        if resp:
            ...
            print("--------列表页返回错误----------{}".format(resp.text))
    ...
    def _save_data(self, data):
        ...
        :return:
        """
        shi_xin_set = self.db.anjuke_2
        res = shi_xin_set.insert(data)
    ...
    def _paras_detail_page(self, items):
        ...
        for item in items:
            ...
            if "1" in floor or "一" in floor:
                ...
                resp_detail = self.__get(url=page_url, headers=random.choice(headers))
                if not resp_detail:
                    continue
                ...
                try:
                    ...
                    data["floor"] = html.xpath(
                        "//div[@id='fy_info']/ul[@class='litem']/li[4]/span[@class='desc']/text()")[0].strip()  # 楼层

                    data["size"] = html.xpath(
                        "//div[@id='fy_info']/ul[@class='ritem']/li[1]/span[@class='desc']/text()")[0].strip()  # 面积

                    data["property_company"] = html.xpath(
                        "//div[@class='itemCon clearfix']/ul[@class='litem']/li[2]/span[@class='desc']/text()")[
                        0].strip()  # 物业公司

                    data["property_fee"] = html.xpath(
                        "//div[@class='itemCon clearfix']/ul[@class='litem']/li[3]/span[@class='desc']/text()")[
                        0].strip()  # 物业费
                    ...
                    print("--------数据获取失败{}:{} ---------".format(e.args, page_url))
                    continue
            else:
    ...
    def run(self):
        ...
        while self.r.exists(REDIS_KEY):
            url = self.r.rpop(REDIS_KEY).decode("utf-8")
            print("-----------------  开始爬取url:%s  ------------------------" % url)
            time.sleep(random.randint(2, 7))
            # proxy = {"https://": ""}
            # session.proxies = proxy
            resp = self.__get(url=url, headers=random.choice(headers))
            if not resp:
                continue
