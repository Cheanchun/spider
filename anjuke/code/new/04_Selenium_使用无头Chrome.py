"""
需求：
    使用 Selenium + 无界面Chrome 访问百度首页，搜索关键字，输出渲染后源码，保存网页快照。
"""
...
# 通过Chrome浏览器组件创建浏览器对象。
# 其中浏览器组件的位置，可以通过可执行文件路径参数指定，也可以通过环境变量指定。
option = webdriver.ChromeOptions()
option.headless = True
# option.add_argument('headless')
# driver = webdriver.Chrome(executable_path="/Users/hero/Downloads/chromedriver", chrome_options=option)
